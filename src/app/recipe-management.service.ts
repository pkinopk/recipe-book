...
export class RecipeManagementService {
  ...
  saveRecipe() {
    if (
      ...
      this.formBinding.instructions.toString() !== '' &&
      this.createIngredients().length !== 0
    ) {
  ...
  edit(recipe) {
    ...
    for (let i = 0; i < this.selectedItem.ingredients.length; i++) {
      this.ingredientsCount.push(
        new Ingredient(
          this.selectedItem.ingredients[i].name,
          this.selectedItem.ingredients[i].quantity
        )
      );
    }
    this.selectedItem = null;
  ...
  getRecipesList(): Promise<any> {
    const request = this.http
      .request(
        new HttpRequest(
          'GET',
          'https://pkinopk-server.herokuapp.com/recipe-book/recipelist'
        )
      )
      .toPromise()
      .catch(reason => {
  ...
  getRecipe(id): Promise<any> {
    const request = this.http
      .request(
        new HttpRequest(
          'GET',
          'https://pkinopk-server.herokuapp.com/recipe-book/retrieverecipe/' +
            id
        )
      )
      .toPromise()
  ...
  addRecipe(recipe: Recipe) {
    ...
    this.http
      .post(
        'https://pkinopk-server.herokuapp.com/recipe-book/addrecipe',
        recipe
      )
      .toPromise()
      .catch(reason => {
  ...
  deleteRecipe(id) {
    ...
    this.http
      .delete(
        'https://pkinopk-server.herokuapp.com/recipe-book/deleterecipe/' + id
      )
      .toPromise()
      .catch(reason => {
        console.log('recipe failed', reason);
